...
int nAnonymizeGold_HawgsAmount = 1000;
...
#else
    const char* pszModule = "Gold_Hawgs";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Gold_Hawgs
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Gold_Hawgs
    // Mac: ~/Library/Application Support/Gold_Hawgs
    // Unix: ~/.Gold_Hawgs
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Gold_Hawgs";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Gold_Hawgs.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Gold_Hawgsd.pid"));
