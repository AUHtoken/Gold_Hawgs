...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeGold_HawgsAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeGold_HawgsAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeGold_HawgsAmount = strAnonymizeGold_HawgsAmount.remove(strAnonymizeGold_HawgsAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeGold_HawgsAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeGold_HawgsAmount*COIN) nMaxToAnonymize = nAnonymizeGold_HawgsAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeGold_HawgsAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeGold_HawgsAmount));
        strAnonymizeGold_HawgsAmount = strAnonymizeGold_HawgsAmount.remove(strAnonymizeGold_HawgsAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeGold_HawgsAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeGold_HawgsAmount == 0){
