...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeGold_HawgsAmount"))
        settings.setValue("nAnonymizeGold_HawgsAmount", 1000);
    nAnonymizeGold_HawgsAmount = settings.value("nAnonymizeGold_HawgsAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeGold_HawgsAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeGold_HawgsAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeGold_HawgsAmount:
            return QVariant(nAnonymizeGold_HawgsAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeGold_HawgsAmount:
            nAnonymizeGold_HawgsAmount = value.toInt();
            settings.setValue("nAnonymizeGold_HawgsAmount", nAnonymizeGold_HawgsAmount);
            emit AnonymizeGold_HawgsAmountChanged(nAnonymizeGold_HawgsAmount);
            break;
        default:
            break;
        }
