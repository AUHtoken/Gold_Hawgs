...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Gold_Hawgs"))
        return false;
...
{
    // Convert Gold_Hawgs:// to Gold_Hawgs:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Gold_Hawgs://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Gold_Hawgs:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Gold_Hawgs.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Gold_Hawgs.lnk
...
{
    return GetAutostartDir() / "Gold_Hawgs.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Gold_Hawgs.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Gold_Hawgs\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Gold_Hawgs app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Gold_Hawgs app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Gold_Hawgs-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Gold_Hawgs-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Gold_Hawgs-Qt"));
