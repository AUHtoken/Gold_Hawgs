...
void Shutdown()
{
    ...
    RenameThread("Gold_Hawgs-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Gold_Hawgs.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Gold_Hawgsd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeGold_Hawgsamount=<n> " + _("Keep N Gold_Hawgs anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Gold_Hawgs is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Gold_Hawgs is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Gold_Hawgs version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Gold_Hawgs server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Gold_Hawgs") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Gold_Hawgs to complete") << "\n";
    ...
    nAnonymizeGold_HawgsAmount = GetArg("-anonymizeGold_Hawgsamount", 0);
    if(nAnonymizeGold_HawgsAmount > 999999) nAnonymizeGold_HawgsAmount = 999999;
    if(nAnonymizeGold_HawgsAmount < 2) nAnonymizeGold_HawgsAmount = 2;
    ...
    LogPrintf("Anonymize Gold_Hawgs Amount %d\n", nAnonymizeGold_HawgsAmount);
